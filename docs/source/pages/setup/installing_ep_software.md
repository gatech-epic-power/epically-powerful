...
EpicallyPowerful is hosted on PyPi (aka pip), and can be installed simply with
```console
...
:::{tip} 
If you get an error that says something like "editable install is not supported", run the command `pip install pip -U`, and then retry `pip install -e .`
:::
