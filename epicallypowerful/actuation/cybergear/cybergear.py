...
import functools
import epicallypowerful.actuation.cybergear.cybergear_driver as cgd

class CyberGear(can.Listener, Actuator):
    ...
    def on_message_received(self, msg: can.Message) -> None:
        ...
        if (target_id != cgd.MASTER_CAN_ID) and (target_id != cgd.RESPONSE_IDENTITY_CHECK_FLAG): return -1

        if communication_type == cgd.RESPONSE_IDENTITY: # This is the response to the identity check
            unique_id, can_id = cgd.parse_identity_response(msg)
            if can_id != self.can_id: return -1
            self.data.unique_hardware_id=unique_id

        if communication_type == cgd.RESPONSE_PARAM:
            param_index, data_value, can_id = cgd.parse_param_response(msg)
            if can_id != self.can_id: return -1
            self.data.internal_params[param_index] = data_value

        if communication_type == cgd.RESPONSE_FAULT: return -1

        if communication_type == cgd.RESPONSE_MOTION:
            pos, vel, trq, temp, can_id, motor_mode, cal_fault, hall_enc_fault, mag_enc_fault, overtemp_fault, overcurr_fault, undervolt_fault = cgd.parse_motion_response(msg)
            if can_id != self.can_id: return -1
    ...
    def _ping_actuator(self) -> None:
        self._bus.send(cgd.create_read_device_id_message(self.can_id))

    def _send_motion_command(self, can_id: int, position: float, velocity: float, torque: float, kp: float, kd: float) -> int:
        self._cybergear_listener.data.commanded_position = position
        self._cybergear_listener.data.commanded_velocity = velocity
        self._cybergear_listener.data.commanded_torque = torque
        self._cybergear_listener.data.kp = kp
        self._cybergear_listener.data.kd = kd

        motion_message = cgd.create_motion_message(target_motor_id=can_id, position=position, velocity=velocity, kp=kp, kd=kd, torque=torque)
    ...
    def _enable(self) -> None:
        ...
        """
        self._ping_actuator()
        enable_motion_message = cgd.create_enable_motion_message(target_motor_id=self.can_id)
        self._bus.send(enable_motion_message, timeout=0.1)
    ...
    def _disable(self) -> None:
        """Disables the motor
        """
        disable_motion_message = cgd.create_disable_motion_message(target_motor_id=self.can_id)
        self._bus.send(disable_motion_message, timeout=0.1)
        self._connection_established = False
